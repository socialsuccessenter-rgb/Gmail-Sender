...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "📊 আপনি প্রতিদিন ৩০০টি ইমেইল পাঠাতে পারবেন।\n\n"
...
async def send_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if "|" not in update.message.text:
        await update.message.reply_text("❌ <b>ভুল ফরম্যাট!</b>", parse_mode=ParseMode.HTML)
        return
    ...
    try:
        ...
        # ফাইল চেক এবং ইমেইল পড়া
        if not os.path.exists('emails.txt'):
            await msg.edit_text("❌ <b>এরর:</b> <code>emails.txt</code> ফাইলটি খুঁজে পাওয়া যায়নি।")
            return

        with open('emails.txt', 'r') as f:
            emails = [line.strip() for line in f.readlines() if line.strip() and "@" in line]

        if not emails:
            await msg.edit_text("❌ <b>এরর:</b> আপনার <code>emails.txt</code> ফাইলটি খালি!")
            return
